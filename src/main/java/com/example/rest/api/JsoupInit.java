...
public class JsoupInit {
	...
	public ArrayList<JSONObject> initialise_popular_movies() throws IOException{ 
		ArrayList<JSONObject> list=new ArrayList<JSONObject>();
	...
	//TODO top 250 shows
		public ArrayList<JSONObject> initialise_top_shows() throws IOException{ 
			ArrayList<JSONObject> list=new ArrayList<JSONObject>();
			Document doc = Jsoup.connect("https://www.imdb.com/chart/toptv/?ref_=nv_tvv_250").get();
			Elements elements=doc.select("tbody.lister-list");
			for(Element e:elements.select("tr")) {
				 var movie = new JSONObject();
				 int rank = Integer.parseInt(e.select("td.titleColumn").text().split(" ")[0].replaceAll("\\p{Punct}", ""));
		...
		//TODO most popular shows
		public ArrayList<JSONObject> initialise_popular_shows() throws IOException{ 
			ArrayList<JSONObject> list=new ArrayList<JSONObject>();
			Document doc = Jsoup.connect("https://www.imdb.com/chart/tvmeter/?ref_=nv_tvv_mptv").get();
			Elements elements=doc.select("tbody.lister-list");
			for(Element e:elements.select("tr")) {
				 var movie = new JSONObject();
				int rank = Integer.parseInt(e.select("td.titleColumn div.velocity").text().split(" ")[0]);
